...
const OutletFinder = () => {
  ...
  const [ifAddress, setIfAddress] = useState(
    localStorage.getItem("outlet") ? true : false
  );
  const handleAddressSelect = (latitude, longitude, address) => {
    ...
    //debugger;
    axios
      .get("https://pizzahaven.store/gateway/outlet/nearest", {
        ...
        >
          ...
          {!showPopup && outlet ? (
            <div className="bg-white p-6 rounded-lg shadow-lg w-96">
              ...
              <p>
                {outlet?.street}, {outlet?.city}, {outlet?.state},{" "}
                {outlet?.zipCode}, {outlet?.country}
              </p>
              <button
                className="w-full bg-green-500 text-white p-2 rounded"
                onClick={() => setIfAddress(true)}
              >
                Ok
              </button>
            </div>
